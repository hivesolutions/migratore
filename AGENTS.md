...
To run the custom suite of unit test for Migratore use the following sequence of commands that will install dependencies
...
- When creating dicts in Python prefer `dict()` over `{}`.
...
- Increment (look at `CHANGELOG.md` for semver changes) the `info.VERSION` value in `src/info.py`.
...
Migratore is licensed under the [Apache License, Version 2.0](http://www.apache.org/licenses/).
